package com.laserdragonuniversity.alpaca {
	...
	public class Inventory extends MovieClip{
		...
		public function addInvItem(itemName:String):void{
			...
			if (playerItems.length < 2){ // This is for the top row of up to 8 items
				addedItem.y = 270;
				addedItem.x = 285 + (playerItems.length) * 61;
			} else { // This creates a bottom row if the player has more than 8 items
				addedItem.y = 330;
				addedItem.x = 285 + (playerItems.length - 2) * 61;
			}
		...
		public function removeInvItem(itemName:String):void{
			...
			for (i in playerItems){
				if (i >= itemNum){
					if (playerItems[i].y == 330 && playerItems[i].x == 285){
						playerItems[i].x = 346;
						playerItems[i].y = 270;
					} else {
						playerItems[i].x -= 61;
